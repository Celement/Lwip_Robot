...
const char* WAY_CGI_Handler (int iIndex,int iNumParams,char *pcParam[],char *pcValue[]);
...
void DS18B20_Handler(char *pcInsert)
{
char Digit1=0, Digit2=0, Digit3=0, Digit4=0,Digit5=0; 
	  short DTemperate = 0;
	  DTemperate=DS18B20_Get_Temp();
	  Digit1 =  DTemperate / 1000;
	  Digit2 =  (DTemperate % 1000)/100;
    Digit3 =  (DTemperate % 100)/10 ;
    Digit4 = (DTemperate % 10)/1;
    Digit5 =  DTemperate % 10;
	//添加到html中的数据
	*pcInsert 		= (char)(Digit1+0x30);
	*(pcInsert+1) = (char)(Digit2+0x30);
	*(pcInsert+2)	=	(char)(Digit3+0x30);
...
void DHT11_Handler(char *pcInsert)
{ u8 temperature;  	    
	u8 humidity;  
 char Digit1=0, Digit2=0, Digit3=0, Digit4=0,Digit5=0; 
 ...
 DHT11_Read_Data(&temperature,&humidity);
	...
	Digit1 = humidity / 1000;
	  Digit2 = (humidity % 1000)/100;
    Digit3 = (humidity % 100)/10 ;
    Digit4 = (humidity % 10)/1;
    Digit5 =  humidity % 10;
	//添加到html中的数据
	*pcInsert 		= (char)(Digit1+0x30);
	*(pcInsert+1) = (char)(Digit2+0x30);
...
static u16_t SSIHandler(int iIndex,char *pcInsert,int iInsertLen)
{
	switch(iIndex)
	{
		...
		case 3:
				...
				break;
		case 4:
			  DS18B20_Handler(pcInsert);
        break;
    case 5:
         DHT11_Handler(pcInsert);
        break;		
...
const char* LEDS_CGI_Handler(int iIndex, int iNumParams, char *pcParam[], char *pcValue[])
{
	...
	if(LED1 == 0 && BEEP == 0) 		return   "/STM32F407LED_OFF_BEEP_OFF.shtml";  	//LED1开,BEEP关
	else if(LED1 == 0 && BEEP == 1) return "/STM32F407LED_OFF_BEEP_OFF.shtml";	//LED1开,BEEP开
	else if(LED1 == 1 && BEEP == 1) return "/STM32F407LED_OFF_BEEP_OFF.shtml";	//LED1关,BEEP开
...
{
	u8 i=0;
	iIndex = FindCGIParameter("way",pcParam,iNumParams);  //找到BEEP的索引号
	if(iIndex != -1) //找到BEEP索引号
	{
		BEEP=0;  //关闭
		for(i = 0;i < iNumParams;i++)
		{
			if(strcmp(pcParam[i],"way") == 0)  //查找CGI参数
			{
				if(strcmp(pcValue[i],"wayleft") == 0) //打开BEEP
				 CAR_LEFT();
				else if(strcmp(pcValue[i],"wayright") == 0) //关闭BEEP
					CAR_RIGHT();
				else if(strcmp(pcValue[i],"wayup") == 0)
					CAR_UP();
				else if(strcmp(pcValue[i],"waydown") == 0)
					CAR_BACK();
				else if(strcmp(pcValue[i],"waystop") == 0)
					CAR_STOP();
			}
		}
	}
